...
flow_ld <- function(staName,sdate,edate,wy){
  ...
  dbkeys <- dbkeys.Q.read() # from flowFormatter.R
  ...
  # ******************************
  if (FALSE) { # skip these lines, dbkeys.Q.read includes these lines
  # convert to lattitude & longitude degrees-minutes-seconds to decimal degrees
    ...
    names(dbkeys)[1] <- 'DBKEY' # change 'Timeseries' to 'DBKEY'
  ...
  api_url <- paste0(
    'https://api.sfwmd.gov/v1/insights-data/cont/data?timeseriesIds=',
    dbkeySelect,
    '&reportType=timeseries&format=json&startDate=',
    sdate.f,'&endDate=',edate.f)  
